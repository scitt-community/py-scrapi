    ...
    def register_signed_statement(self, statement):
        ...
        try:
            event = self._archivist.post(
                f'{self._url}/archivist/v1/events',
                payload
            )
    ...
    def check_registration(self, registration_id):
        ...
        try:
            event = self._archivist.get(
                f'{self._url}/archivist/v1/{registration_id}'
            )
        except ArchivistBadRequestError as e:
            logging.debug(e)
            return None, encode_problem_details(
...
#FIXME POINT-GET THE EVENT AND DIG OUT THE STATEMENT FROM ATTRIBUTES
    def resolve_signed_statement(self, entry_id):
        ...
        try:
            event = self._archivist.get(
                f'{self._url}/archivist/v1/{entry_id}'
            )
        except ArchivistBadRequestError as e:
            logging.debug(e)
            return None, encode_problem_details(
        ...
        if not event["attributes"]["signed_statement"]:
            return None, encode_problem_details(
                {
                    "type": "https://example.com/error/not-found-error",
        ...
        b64_statement = event["attributes"]["signed_statement"]
